...
export default function PlayersPage() {
  const { t } = useI18n();
  const { athletes, addChild, setActiveChild } = useFamily();
    ...
    <PortalShell>
      <div>
          ...
          {athletes.map((athlete) => (
            <Card key={athlete.id} style={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              <CardContent>
                <div style={{ display: "flex", alignItems: "flex-start", gap: "16px", marginBottom: "16px" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: "0.8125rem", color: "#64748B", marginTop: "4px" }}>
                      Born: {athlete.dateOfBirth} • {athlete.gender ?? "Youth Athlete"}
                    </div>
                  </div>
                </div>
                ...
                >
                  <div>🏀 Primary Sport: <strong>Basketball</strong></div>
                  <div>👥 Relationship: <strong>{athlete.relationshipType ?? "Guardian"}</strong></div>
                  <div>🌐 Preferred Locale: <strong>{athlete.preferredLocale.toUpperCase()}</strong></div>
                </div>
        ...
        <Dialog
          ...
          description="Create a managed child profile linked to your guardian account."
        >
