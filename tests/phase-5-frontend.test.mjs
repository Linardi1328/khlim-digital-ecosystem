...
test(
  "Phase 5 enrolment filters programmes by birth-year cohort and guards child creation",
  async () => {
    const enrol = await read("apps/web/app/enrol/page.tsx");
...
test("Phase 5 registration handles email-confirmation mode without pretending the user is signed in", async () => {
  ...
  assert.match(authContext, /emailConfirmationOrSignInRequired/);
  ...
  assert.match(catalogue, /Check your email or sign in/);
  assert.match(
    catalogue,
